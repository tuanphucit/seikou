...
if ($fee->checkAccess($module->authAssignmentsManagerRole) || $fee->checkAccess($module->authItemsManagerRole)):
	echo '<h2>'.Yii::t('RbamModule.rbam','Select a task').'</h2>';
	echo '<ul>';
	if ($fee->checkAccess($module->authAssignmentsManagerRole)):
		echo '<li>'.CHtml::link(Yii::t('RbamModule.rbam','Manage Authorisation Assignments').' &#187;',array('authAssignments/index')).'</li>';
	endif;
	if ($fee->checkAccess($module->authItemsManagerRole)):
		echo '<li>'.CHtml::link(Yii::t('RbamModule.rbam','Manage Authorisation Items').' &#187;',array('authItems/index')).'</li>';
	endif;
	if ($module->development && $fee->checkAccess($module->rbacManagerRole)):
